  ...
  class MessageSender
    ...
    def deliver_through_app
      ...
      @message_delivery.device_registrations.each do |device_registration|
        amazon_sns.send_message device_registration.platform, device_registration.endpoint_arn, @message_delivery.text, @message_delivery.message.article.thumb_picture(:thumb).url
      end
      true
    end
  end
