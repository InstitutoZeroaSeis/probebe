require 'rails_helper'

RSpec.describe ChildrenFinder, :type => :model do

  context "with a article targeting born, males until 24 months" do
    context "and with a born, male and half an year old" do
      subject { ChildrenFinder.new(@article, Child.all).find_childrens_for_article }
      before { @child = create(:child, birth_date: 18.months.ago) }
      before { @article = create(:article, :male, maximum_valid_week: 24 * 7, baby_target_type: 'born') }
      it { is_expected.to match_array([@child]) }
    end
  end

  context "with a article targeting born children, male that are between 1 and 2 years old" do
    context "and with a male baby about 18 months old and another baby, female about 14 months old" do
      subject { ChildrenFinder.new(@article, Child.all).find_childrens_for_article }
      before { @male = create(:child, :male, birth_date: (18.months).ago) }
      before { @female =  create(:child, :female, birth_date: (14.months).ago) }
      before { @article = create(:article, :male, minimum_valid_week: 52, maximum_valid_week: 104, baby_target_type: 'born') }
      it { is_expected.to match_array([@male]) }
    end
  end

  context "with a article targeting pregnancies between 30 and 35 weeks" do
    context "and with a born baby aged 32 weeks" do
      subject { ChildrenFinder.new(@article, Child.all).find_childrens_for_article }
      before { @article = create(:article, minimum_valid_week: 30, maximum_valid_week: 35, baby_target_type: 'pregnancy') }
      before { @child = create(:child, birth_date: 32.weeks.ago) }
      it { is_expected.to match_array([]) }
    end
  end

  context "with a article targeting pregnancies between 30 and 35 weeks" do
    context "and with a baby with about 32 weeks of pregnancy" do
      subject { ChildrenFinder.new(@article, Child.all).find_childrens_for_article }
      before { @article = create(:article, minimum_valid_week: 30, maximum_valid_week: 35, baby_target_type: 'pregnancy') }
      before { @child = create(:child, birth_date: (Child::PREGNANCY_DURATION_IN_WEEKS - 32).weeks.from_now) }
      it { is_expected.to match_array([@child]) }
    end
  end

end
